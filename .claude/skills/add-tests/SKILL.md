...
スタブに存在しない Protocol を新たにテストするときは `TestDoubles.swift` に追記する。
...
struct CityListViewModelTests {
    private func makeFreshViewModel() -> CityListViewModel {
        // CityListViewModel は UserDefaults.standard を直接使うため毎回クリア
        UserDefaults.standard.removeObject(forKey: "registeredCities")
        return CityListViewModel(repository: StubWeatherRepository())
    }
